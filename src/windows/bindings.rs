...
impl ::std::fmt::Debug for _MIB_IPFORWARD_ROW2 {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write!(
            f,
            "_MIB_IPFORWARD_ROW2 {{ InterfaceLuid: {:?}, InterfaceIndex: {:?}, DestinationPrefix: \
             {:?}, NextHop: {:?}, SitePrefixLength: {:?}, ValidLifetime: {:?}, PreferredLifetime: \
             {:?}, Metric: {:?}, Protocol: {:?}, Loopback: {:?}, AutoconfigureAddress: {:?}, \
             Publish: {:?}, Immortal: {:?}, Age: {:?}, Origin: {:?} }}",
            self.InterfaceLuid,
            self.InterfaceIndex,
            self.DestinationPrefix,
            self.NextHop,
            self.SitePrefixLength,
            self.ValidLifetime,
            self.PreferredLifetime,
            self.Metric,
            self.Protocol,
            self.Loopback,
            self.AutoconfigureAddress,
            self.Publish,
            self.Immortal,
            self.Age,
            self.Origin
